 ...
 */
  setMotion(m) {
    ...
    console.log('exp', exp, this.vrm, m);
 ...
 */
      const b = m.b[k];
      const rawbone = humanoid.getNormalizedBone(k);
      if (!rawbone) {
        console.warn('rawbone', k);
        continue;
      }
      rawbone.node.rotation.x = b[0];
      rawbone.node.rotation.y = b[1];
      rawbone.node.rotation.z = b[2];
    }
...
/**
 * 
 * @param {{targetname: string, erot: [number,number,number], order?: string}} data 
 */
  setOneJoint(data) {

    const motions = this.motion.motions;
    motions[data.targetname] = data;
